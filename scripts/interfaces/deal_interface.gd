...
func upgrade_handler(type: String) -> void:
	...
	if player != null:
		match type:
			...
			"double crit chance":
				current_stat_label.text = "current crit % :" + str(player.current_crit_chance)
				double_stat_label.text = "Double crit % :" + str(player.current_crit_chance * 2)
		...
		refresh_timer.start() # or refresh_timer.start(0.3) if you set One Shot
